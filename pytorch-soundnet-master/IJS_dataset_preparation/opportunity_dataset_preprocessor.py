...
def get_attribute_names(root_path, window_size, overlay):

    # make attribute names and labels
    att_names = ['MILLISEC']
    label_names = ['Subject']
    collumn_name_file = os.path.join(root_path, "OpportunityUCIDataset/dataset/column_names.txt")
    index = 1
...
#TODO
def load_set(root, window_size, overlay):

    path = os.path.join(root, "OpportunityUCIDataset/dataset")
    activities = []
    data = []
    
    atts = get_attribute_names(root, window_size, overlay) #get an array of attribute names
    ...
    for i in range(1,5):
        ...
        for j in range(1,7):
            ...
            #load data
            raw_data = pd.read_csv(os.path.join(path, fname) + '.dat', sep=' ')
            del fname
            ...
            preprocessed_path = os.path.join(root, "OpportunityUCIDataset/Subject_")
            pickle.dump(data, open(preprocessed_path+str(i) + '-ADL' + str(j) + '_preprocessed.txt','wb'), pickle.HIGHEST_PROTOCOL)
...
def concat_frames(root):
    preprocessed_path = os.path.join(root, "OpportunityUCIDataset/Subject_")
    flag = 0
    
    #TODO -> (1,5)
    for i in range(1,5):  #1 2 3 4 subject
        for j in range(1,7):  #1 2 3 4 5 6  activity
            print(str(i) + " - " + str(j))
            temp_data = pickle.load(open(preprocessed_path + str(i) + '-ADL' + str(j) + '_preprocessed.txt', "rb"))
            if flag == 0:
    ...
    # saving the file
    preprocessed_path = os.path.join(root, "full_preprocessed_dataset.txt")
    pickle.dump(data, open(preprocessed_path,'wb'), pickle.HIGHEST_PROTOCOL)
...
#OK 
def preprocess(root, window_size, overlay):
    
    if os.path.exists(os.path.join(root, "OpportunityUCIDataset/dataset")):
        #load_set(root, window_size, overlay)
        concat_frames(root)
