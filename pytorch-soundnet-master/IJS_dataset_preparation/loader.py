...
class Loader:
    root = os.path.join(os.path.abspath(os.path.join(os.getcwd(), "../..")),"Dataset")
    dataset_name = ""
    dataset_zip = ""
    dataset_url = ""
    dataset_unzip = ""
...
#OPPORTUNITY dataset
    ...
    def opportunity_load(self):
        ...
        self.dataset_unzip = self.root + '//'+"OpportunityUCIDataset"
    ...
    def opportunity_preprocess(self, window_size, overlay):
        ...
        return opportunity.preprocess(self.root, window_size, overlay)
...
if __name__ == '__main__':
   ...
   sliding_window = 5
   overlay = 1
   loader.opportunity_load()
   loader.opportunity_preprocess(sliding_window, overlay) # 300, 80

   df = pickle.load(open('C:\ALEX\Doc\Reference\SoundNet\PeriodicalLearning\Dataset\\full_preprocessed_dataset.txt', 'rb'))
   print("------------------------------------------------------")
